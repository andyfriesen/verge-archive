...
The error reporting in VC is quite unpleasant at the moment. It doesn't
even report the line number of the error. I hope to work this out sooner
or later, but I wouldn't count on it happening sooner. But there's not really
many possible ways to generate an error.

- vecna@inlink.com

Some final words from Spram:
<Spram> Oh shit no! I forgot to say: If the game crashes for no reason when you load, then start
a new game and load the game from the Esc Menu.